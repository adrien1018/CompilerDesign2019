...
typedef struct Value{
    ValueType type;
    union{
        char id[kMaxVarLen + 1];   /* if the node represent the access of the identifier */
...
typedef struct Statement{
    StmtType type;
    union{
        char variable[kMaxVarLen + 1];              /* print statement */
